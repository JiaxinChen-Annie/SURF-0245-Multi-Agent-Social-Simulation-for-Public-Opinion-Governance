...
class OpinionModel(Model):
        ...
        self.datacollector = DataCollector(
            model_reporters={
                ...
                "time_mu_factor":       lambda m: m.sim_clock.hawkes_mu_factor(
                                            int(m.schedule.time)),
                # 分角色 / 分群（老师：不要只画全网平均；供 E / plot_run_report）
                "avg_opinion_ORDINARY":   lambda m: m._calc_avg_opinion_for_type(AgentType.ORDINARY),
                "avg_opinion_ACTIVE":     lambda m: m._calc_avg_opinion_for_type(AgentType.ACTIVE),
                "avg_opinion_RATIONAL":   lambda m: m._calc_avg_opinion_for_type(AgentType.RATIONAL),
                "avg_opinion_CONTROLLER": lambda m: m._calc_avg_opinion_for_type(AgentType.CONTROLLER),
                "neg_emotion_ORDINARY":   lambda m: m._calc_negative_for_type(AgentType.ORDINARY),
                "neg_emotion_ACTIVE":     lambda m: m._calc_negative_for_type(AgentType.ACTIVE),
                "neg_emotion_RATIONAL":   lambda m: m._calc_negative_for_type(AgentType.RATIONAL),
                "neg_emotion_CONTROLLER": lambda m: m._calc_negative_for_type(AgentType.CONTROLLER),
                "group_heat_DORM":        lambda m: m._calc_heat_for_group(GroupType.DORM),
                "group_heat_CLASS":       lambda m: m._calc_heat_for_group(GroupType.CLASS),
                "group_heat_MAJOR":       lambda m: m._calc_heat_for_group(GroupType.MAJOR),
                "group_heat_CAMPUS":      lambda m: m._calc_heat_for_group(GroupType.CAMPUS),
                "group_neg_DORM":         lambda m: m._calc_negative_for_group(GroupType.DORM),
                "group_neg_CLASS":        lambda m: m._calc_negative_for_group(GroupType.CLASS),
                "group_neg_MAJOR":        lambda m: m._calc_negative_for_group(GroupType.MAJOR),
                "group_neg_CAMPUS":       lambda m: m._calc_negative_for_group(GroupType.CAMPUS),
            }
        )
    ...
    def _calc_avg_opinion_for_type(self, agent_type: AgentType) -> float:
        """分角色平均观点（老师：角色单独画图）。"""
        vals: List[float] = []
        for agent in list(self.schedule.agents):
            if not hasattr(agent, "beliefs"):
                continue
            if agent.beliefs.identity.agent_type != agent_type:
                continue
            ops = agent.beliefs.opinions
            if not ops:
                continue
            op = ops.get("T001") or list(ops.values())[0]
            vals.append(float(op.opinion_value))
        return float(np.mean(vals)) if vals else 0.0

    def _calc_negative_for_type(self, agent_type: AgentType) -> float:
        """分角色负面情绪强度 (1-valence)/2。"""
        vals: List[float] = []
        for agent in list(self.schedule.agents):
            if not hasattr(agent, "beliefs"):
                continue
            if agent.beliefs.identity.agent_type != agent_type:
                continue
            vals.append((1.0 - float(agent.beliefs.emotion.valence)) / 2.0)
        return float(np.clip(np.mean(vals), 0.0, 1.0)) if vals else 0.0

    def _calc_heat_for_group(self, group_type: GroupType) -> float:
        """分群热度（对所有话题取该群最大热度）。"""
        vals = [gm.get(group_type, 0.0) for gm in self.topic_heat.values()]
        return float(max(vals)) if vals else 0.0

    def _calc_negative_for_group(self, group_type: GroupType) -> float:
        """分群负面程度（对所有话题取该群最大负面）。"""
        vals = [gm.get(group_type, 0.0) for gm in self.topic_negative.values()]
        return float(max(vals)) if vals else 0.0
