...
【B v3.2 记忆（2026-08-02 并入 A 接线版）】
  - 短期明细 deque + 长期摘要压缩（不再仅靠 [-5:]）；
  - Perception 与 Memory 分离：感知经重要性筛选后才写入；
  - 高影响事件固化；虚假/高失真接触可轻微降低 trust（belief 迁移钩子）。
...
_SUMMARY_INTERVAL = 10         # 每 N 次写入尝试压缩一次
_HIGH_IMPACT_THRESHOLD = 0.70  # relevance ≥ 此值 → 高影响固化
_MAX_LONG_TERM_SUMMARIES = 10
_TRUST_POLLUTION_STEP = 0.01   # 接触高失真时 trust 微降（可关）
...
class AgentMemorySystem:
    """短期明细 + 长期摘要 + 高影响事件（接口仍对外暴露 MemoryRecord）。"""
    short_term: deque = field(default_factory=lambda: deque(maxlen=_MEMORY_CAPACITY))
...
class SocialAgent(Agent):
    ...
    def __init__(
        ...
        # B 记忆系统（短期 + 长期摘要）；self.memory 兼容旧代码读 short_term
        self.memory_system = AgentMemorySystem(
            short_term=deque(maxlen=_MEMORY_CAPACITY),
            summary_interval=_SUMMARY_INTERVAL,
        )
        self.memory = self.memory_system.short_term
        self.enable_trust_pollution: bool = bool(
            init_config.get("enable_trust_pollution", True)
        )
    ...
    # ------------------------------------------------------------------ #
    #  #5  _retrieve_memory / 写入 / 压缩（B v3.2）                         #
    # ------------------------------------------------------------------ #
    def _score_memory_importance(self, si: SocialInfo, *, own: bool = False) -> float:
        """感知→记忆重要性：高负面/高失真/提及/本人行动更易固化。"""
        score = 0.35
        score += 0.35 * float(getattr(si, "negative_score", 0.0) or 0.0)
        score += 0.25 * float(getattr(si, "distortion_level", 0.0) or 0.0)
        score += 0.15 * float(getattr(si, "heat", 0.0) or 0.0)
        if getattr(si, "is_mention", False):
            score += 0.20
        if own:
    ...
    def _write_memory(self, record: MemoryRecord) -> None:
        """写入短期记忆；高影响固化；周期性压缩为长期摘要。"""
        ms = self.memory_system
        ms.write_count += 1
        ms.short_term.append(record)
        if record.relevance >= _HIGH_IMPACT_THRESHOLD:
            ms.high_impact_events.append(record)
        if (
            ms.write_count % max(1, ms.summary_interval) == 0
            and len(ms.short_term) >= 5
        ):
    ...
    def _compress_memory(self) -> None:
        """短期明细 → 长期摘要（模糊长久记忆）。"""
        ms = self.memory_system
        entries = list(ms.short_term)[-ms.summary_interval :]
        if len(entries) < 3:
            return
        key_topics = sorted({
            (e.info.topic_id or "") for e in entries if getattr(e.info, "topic_id", "")
        })
        key_events = [
            str(getattr(e.info, "content", "") or "")[:40]
            for e in entries if e.relevance >= 0.5
        ][:3]
        start_tick = int(entries[0].tick)
        end_tick = int(entries[-1].tick)
        summary = LongTermSummary(
            tick_range=(start_tick, end_tick),
            summary=(
                f"第{start_tick}-{end_tick}步: {len(entries)}条记忆"
                + (f"；要点:{'｜'.join(key_events)}" if key_events else "")
            ),
            key_topics=[t for t in key_topics if t][:3],
            key_events=key_events,
        )
        ms.long_term_summaries.append(summary)
        if len(ms.long_term_summaries) > _MAX_LONG_TERM_SUMMARIES:
            ms.long_term_summaries = ms.long_term_summaries[-_MAX_LONG_TERM_SUMMARIES:]

    def _ingest_social_info(
        self,
        si: SocialInfo,
        tick: int,
        *,
        ...
        """Perception 与 Memory 分离：低重要性不落盘。"""
        importance = self._score_memory_importance(si, own=own)
        if importance < min_importance and not own:
            return
        self._write_memory(MemoryRecord(tick=tick, info=si, relevance=importance))

    def _apply_trust_pollution(self, perception: Perception) -> None:
        """虚假/高失真累积 → trust 轻微下降（belief 迁移，默认开启小步长）。"""
        if not self.enable_trust_pollution:
            return
        psych = self.beliefs.psychology
        n_bad = sum(
            1 for si in perception.recent_messages
            if float(getattr(si, "distortion_level", 0.0) or 0.0) >= 0.5
        )
        if n_bad <= 0:
            return
        # ORDINARY / ACTIVE 更易被污染；RATIONAL / CONTROLLER 更稳
        role = self.beliefs.identity.agent_type
        scale = {
            AgentType.ORDINARY: 1.0,
            AgentType.ACTIVE: 0.7,
            AgentType.RATIONAL: 0.25,
            AgentType.CONTROLLER: 0.2,
        }.get(role, 0.5)
        delta = _TRUST_POLLUTION_STEP * scale * min(n_bad, 3)
        psych.trust = float(np.clip(psych.trust - delta, 0.05, 1.0))

    def _retrieve_memory(self, perception: Perception) -> List[MemoryRecord]:
        """短期最近条 + 高影响 + 匹配话题的长期摘要（供 C prompt）。"""
        retrieved: List[MemoryRecord] = []
        retrieved.extend(list(self.memory_system.short_term)[-8:])
        retrieved.extend(list(self.memory_system.high_impact_events)[-3:])

        active_topics = set((perception.topic_heat or {}).keys())
        summaries = self.memory_system.long_term_summaries[-3:]
        for i, summary in enumerate(summaries):
            matched = (not active_topics) or bool(set(summary.key_topics) & active_topics)
            # 无匹配时仍保留最近一条摘要作模糊背景
            if not matched and i < len(summaries) - 1:
                continue
            retrieved.append(MemoryRecord(
                tick=int(summary.tick_range[1]),
                info=SocialInfo(
                    source_id=-1,
                    source_nickname="memory_summary",
                    content=f"[摘要] {summary.summary}",
                    topic_id=(summary.key_topics[0] if summary.key_topics else ""),
                    timestamp=int(summary.tick_range[1]),
                ),
                relevance=0.35,
            ))
        ...
        for rec in retrieved:
            key = (rec.tick, (rec.info.content or "")[:24])
            if key in seen:
                continue
            seen.add(key)
            unique.append(rec)
        ...
        if llm_mod is not None and llm_client is not None:
            try:
                ...
                env_info = {
                    ...
                    "memory_summary": [
                        (m.info.content or "")[:80]
                        for m in memories
                        if (m.info.content or "").startswith("[摘要]")
                    ][:3],
                    "trust": float(self.beliefs.psychology.trust),
                }

                # ── 调用 C 模块三个接口（窗口预算 + per-agent user_id）──
                llm_cfg = getattr(getattr(self.model, "config", None), "llm_config", {}) or {}
                n_recent = int(llm_cfg.get("max_recent_messages", 6))
                n_mention = min(3, n_recent)
                env_info["recent_messages"] = [
                    {
                    ...
                    for si in perception.recent_messages[:n_recent]
                ]
                env_info["mentions"] = [
                    {
                        "message_id": si.message_id,
                    ...
                    for si in perception.mentions[:n_mention]
                ]
                budget = {
                    "max_memory_items": int(llm_cfg.get("max_memory_items", 4)),
                    "max_recent_messages": n_recent,
                    "max_prompt_chars": int(llm_cfg.get("max_prompt_chars", 6000)),
                }
                prompt = llm_mod.build_prompt(
                    self.beliefs, list(memories), env_info, budget=budget
                )
                uid = f"agent-{int(self.unique_id)}"
                try:
                    raw = llm_client.chat(prompt, user_id=uid)
                except TypeError:
                ...
                for si in perception.recent_messages[:5]:
                    self._ingest_social_info(si, perception.tick)
                for si in perception.mentions[:3]:
                    self._ingest_social_info(si, perception.tick, min_importance=0.2)
                self._apply_trust_pollution(perception)
