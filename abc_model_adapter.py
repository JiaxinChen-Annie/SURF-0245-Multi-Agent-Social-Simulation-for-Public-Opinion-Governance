...
from strategy_eval import (
    InterventionType,
    _governance_tick,
    _segment_columns_from_global,
    _time_profile_multiplier,
)
...
class ABCOpinionModelAdapter:
    ...
    def run(self, steps: int) -> pd.DataFrame:
        ...
        for tick in range(int(steps)):
            if tick == governance_tick:
                self._apply_initial_intervention(model, tick)
            model.step()
    ...
    def _apply_initial_intervention(self, model: OpinionModel, tick: int) -> None:
        ...
        if self.intervention_type == InterventionType.NODE_CONTROL:
            ...
            for group_type in GroupType:
                if group_type != GroupType.DORM:
                    model.intervention_tick[group_type] = int(tick)
    ...
    def _to_strategy_metrics(
        ...
        metrics = _attach_segment_columns(metrics)
        return metrics
    ...
    def _apply_post_metrics_effect(self, metrics: pd.DataFrame) -> pd.DataFrame:
        ...
        governance_tick = _governance_tick(self.intervention_params, len(out))
        active = tick >= float(governance_tick)
        elapsed = np.maximum(0.0, tick - float(governance_tick))
        time_multiplier = np.array(
            [_time_profile_multiplier(int(t), self.intervention_params) for t in tick],
            dtype=float,
        )
        effect_curve = (1.0 - np.exp(-(elapsed + 1.0) / 8.0)) * active * time_multiplier
        ...
        return _attach_segment_columns(out)


def _attach_segment_columns(metrics: pd.DataFrame) -> pd.DataFrame:
    out = metrics.copy()
    segment_rows = [_segment_columns_from_global(row) for row in out.to_dict("records")]
    if not segment_rows:
        return out
    segment_frame = pd.DataFrame(segment_rows, index=out.index)
    for column in segment_frame.columns:
        out[column] = segment_frame[column]
    return out
